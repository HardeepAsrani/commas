...
function commas_enqueue_scripts() {
	wp_enqueue_script( 'commas', plugins_url( '/', __FILE__ ) . 'build/build.js', array( 'wp-i18n', 'wp-blocks', 'wp-block-editor', 'wp-components', 'wp-data', 'wp-element' ), COMMAS_VERSION, true );
}
			...
			array(
				...
				'args'                => array(
					'content' => array(
						'type'        => 'string',
						'required'    => true,
						'description' => __( 'Content of Block.', 'textdomain' ),
					),
				),
...
function commas_check( $request ) {
	...
	$args = array(
		'headers' => array(
			...
			'x-rapidapi-key' => 'YOUR_GRAMMARBOT_API_KEY',
